...
ggplot(time_series, aes(as.factor(year), profit_adjusted_inflation)) +
  ...
  labs(x = "Year",
       ...
       title = "Uncertainty in Inflation-Adjusted Almond Profits Through Time (1989-2010)") 
  theme_minimal()

# Graph 2: almond yields in any one year or for all years together, with uncertainty
#one that show how profit in general varies with your parameters
time_series %>%
  pivot_longer(cols = 6:7, names_to = "parameter", values_to = "sensitivity") %>%
ggplot(aes(price_per_ton, parameter)) +
  geom_boxplot()
